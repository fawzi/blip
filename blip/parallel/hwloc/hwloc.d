 ...
 *
 * the interface wrapped is the one of Revision: 1890
 * wrapping by fawzi
 */
...
struct hwloc_topology_support {
  ...
  enum CpubindFlags:uint{
	  /** Binding the whole current process is supported.  */	  
	  set_thisproc_cpubindMask=0x8000_0000,
	  /** Getting the binding of the whole current process is supported.  */
	  get_thisproc_cpubindMask=0x4000_0000,
	  /** Binding a whole given process is supported.  */
	  set_proc_cpubindMask=0x2000_0000,
	  /** Getting the binding of a whole given process is supported.  */
	  get_proc_cpubindMask=0x1000_0000,
	  /** Binding the current thread only is supported.  */
	  set_thisthread_cpubindMask=0x0800_0000,
          /** Getting the binding of the current thread only is supported.  */
	  get_thisthread_cpubindMask=0x0400_0000,
	  /** Binding a given thread only is supported.  */
	  set_thread_cpubindMask=0x0200_0000,
	  /** Getting the binding of a given thread only is supported.  */
	  get_thread_cpubindMask=0x0100_0000,
	  padMask=0x00FF_FFFF
  }
  uint cpubind;
}
