/// Hashing functions.
/// The main part is code written by others (Austin Appleby for murmurhash,
/// Bob Jenkins for the other hash, worth nothing also Paul Hsieh hash
/// http://www.azillionmonkeys.com/qed/hash.html , even if I did not use it)
/// This code is released as public domain or equivalent, so to respect
/// their work this whole module can also be used under that license.
///
/// author: Fawzi (and others as explained in the comments)
//
...
module blip.util.Hash;

/// hashes bStart[0..length] bytes
hash_t rt_hash_str(const(void) *bStart,size_t length, hash_t seed=0){
...
hash_t rt_hash_block(const(size_t) *bStart,size_t length, hash_t seed=0){
    static if (is(hash_t==uint)||is(hash_t==int)){
        return cast(hash_t)murmurHashAligned2(bStart,length,cast(uint)seed);
    } else static if (is(hash_t==ulong)||is(hash_t==long)){
...
uint rt_hash_utf8(const(char)[] str, uint seed=0){
...
uint rt_hash_utf16(const(wchar)[] str, uint seed=0){
    const uint m = 0x5bd1e995;
    const int r = 24;
...
uint rt_hash_utf32(const(dchar)[] str, uint seed=0){
    const uint m = 0x5bd1e995;
    const int r = 24;
...
uint rt_hash_str_neutral32(const(void) *bStart,uint length, uint seed=0){
    return murmurHashNeutral2(bStart,length,seed);
}

/// hashes bStart[0..length] in an architecture neutral way
ulong rt_hash_str_neutral64(const(void) *bStart,ulong length, ulong seed=0){
...
private uint murmurHashAligned2 ( const(void) * key, uint len, uint seed )
{
    const uint m = 0x5bd1e995;
    const int r = 24;
...
private uint murmurHashNeutral2 ( const(void) * key, uint len, uint seed )
{
    const uint m = 0x5bd1e995;
    const int r = 24;
...
ulong lookup8_hash( const(ubyte)* k, ulong length, ulong level)
{
  ulong a,b,c,len;
...
--------------------------------------------------------------------
*/
ulong lookup8_hash2( const(ulong) *k, ulong length, ulong level)
{
  ulong a,b,c,len;
